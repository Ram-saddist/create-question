...
const ProgramManagement = () => {
  ...
  const [formData, setFormData] = useState({  name: '', email: '', phone: '', location: '', userType: 'manager' });
  ...
  const handleOpenModal = (bde = null) => {
    setFormData(bde || {  name: '', email: '', phone: '', location: locations[0], userType: 'manager' });
    setIsModalOpen(true);
  };
  ...
  const handleCloseModal = () => {
    ...
    setFormData({ name: '', email: '', phone: '', location: '', userType: 'manager' });
  };
  ...
  const handleAdd = async (newData) => {
    console.log('New Manager:', newData);
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BACKEND_URL}/api/v1/manager`,
        newData
      );
  ...
  const handleUpdate = async (id, updatedData) => {
    ...
    try {
      const response = await axios.put(
        `${process.env.REACT_APP_BACKEND_URL}/api/v1/manager/${id}`,
        updatedData
      );
      ...
      Swal.fire({ icon: 'error', title: 'Failed to update manager.' });
    }
  };

  // Save BDE (Add/Edit)
  const handleSave = async () => {
    if (!formData.name || !formData.email || !formData.phone || !formData.location || !formData.userType) {
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`${process.env.REACT_APP_BACKEND_URL}/api/v1/manager/${id}`);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-96">
            ...
            <form className="space-y-4">
              ...
              >
                <option value="bde">manager</option>
              </select>
            </form>
