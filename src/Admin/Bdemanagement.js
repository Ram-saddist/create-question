...
const Bdemanagement = () => {
  ...
  const [formData, setFormData] = useState({  name: '', email: '', phone: '', location: '', userType: 'BDE' });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/v1/bdesignup`);
        setData(response.data.BDE);
        console.log('BDE data:', response.data);
      } catch (error) {
  ...
  const handleOpenModal = (bde = null) => {
    setFormData(bde || {  name: '', email: '', phone: '', location: locations[0], userType: 'BDE' });
    setIsModalOpen(true);
  };
  ...
  const handleCloseModal = () => {
    ...
    setFormData({ name: '', email: '', phone: '', location: '', userType: 'BDE' });
  };
  ...
  const handleAdd = async (newData) => {
    ...
    try {
      ...
      Swal.fire({ icon: 'error', title: 'Failed to add BDE.' });
    }
  };
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-96">
            <h3 className="text-lg font-bold mb-4">{formData.id ? 'Edit BDE' : 'Add BDE'}</h3>
